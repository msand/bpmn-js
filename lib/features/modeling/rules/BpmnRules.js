...
function canAttach(element, target, position) {
  if (is(element, 'bpmn:BoundaryEvent')) {
    if (is(target, 'bpmn:Process') || is(target, 'bpmn:Participant')) {
      return false;
    }
  }

  if (position && !isConnection(target)) {
    if (is(target, 'bpmn:SubProcess')) {
      if (isOutside(position, target, 0.125)) {
        return true;
      }
    }
  }
  ...
  return canDrop(element, target) || canInsert(element, target);
}
...
function canMove(elements, target) {
  ...
  if (elements.length === 1 && is(elements[0], 'bpmn:BoundaryEvent')) {
    return canAttach(elements[0], target);
  }
