...
function BpmnReplace(bpmnFactory, moddle, popupMenu, replace, selection, modeling) {
  ...
  function getToggleOptions(element) {
    ...
    var isSequential,
        isLoop,
        isParallel;
        //isAdHoc = is(element, 'bpmn:AdHocSubProcess'),
    ...
    function toggleMi(event, entry) {
      if (entry.active) {
        ...
        // update entries
        if (entry.id !== 'toggle-parallel-mi') {
          popupMenu.update('toggle-parallel-mi', { active: false });
        }
        if (entry.id !== 'toggle-sequential-mi') { 
          popupMenu.update('toggle-sequential-mi', { active: false });
        }
        if (entry.id !== 'toggle-loop') {
          popupMenu.update('toggle-loop', { active: false });
        }
